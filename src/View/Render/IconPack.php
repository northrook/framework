...
final class IconPack
{
    ...
    // TODO : Implement CacheAdapter
    public function __construct( null|string|array $pack = null )
    {
        if ( null === $pack ) {
            $this->name  = 'IconPack::DEFAULT';
            $this->icons = $this::DEFAULT;
        }
        elseif ( \is_array( $pack ) ) {
            $this->name  = $pack['name'] ?? throw new ValueError( 'Undefined icon pack name in provided array.' );
            $this->icons = $pack;
        }
        else {
            // TODO
    ...
    public function get(
        ...
        [$icon, $data] = Str::bisect( $icon, '[' );

        if ( ! $this->has( $icon ) ) {
            // If no desired Icon, check for fallback

            if ( $fallback && ! $this->has( $fallback ) ) {
                Log::error(
                    'Unable to resolve icon {icon}, no fallback {fallback} found.',
                    ['icon' => $icon, 'fallback' => $fallback],
                );
                return null;
            }
            Log::warning(
                'The icon {icon} was requested, but could not be found in the {name} IconPack, {fallback} used as fallback',
                ['icon' => $icon, 'name' => $this->name, 'fallback' => $fallback],
            );
        }

        $vector = $this->getIcon( $icon, $fallback );

        $attributes = \array_merge( $this->defaultAttributes, $attributes, $vector['attributes'] );
        $svg        = \trim( \preg_replace( ['#\s+#m', '#>\s<#'], [' ', '><'], $vector['svg'] ) );

        $element = new Element( 'svg', $attributes, $svg );

        $element->class( $icon );

        if ( $data ) {
            dump( 'We have [$data] to parse!', $data, $icon, '----' );
            // $data = \trim( $data, " \n\r\t\v\0[]" );
        ...
        return $asElement ? $element : $element->toString();
    }
     ...
     */
    private function getIcon( string $icon, ?string $fallback ) : array
    {
